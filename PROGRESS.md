...
- [~] Session — **SDK lifecycle done**, control-api/dashboard session views not yet
  ...
  - [ ] control-api: no session list/detail endpoints yet (mirrors `mvp.md`'s Session entity — `session_id, started_at, last_seen_at, browser, device` — same ADR-023 GROUP-BY-over-events shape issues/network already use, no materialized table needed since every event already carries `session_id`)
  - [ ] Dashboard: no session views yet, and no link from an issue occurrence to its session (`session-investigation.md`'s documented entry point: "Issue → affected session") — `IssueDetail`'s `recentOccurrences` already carries `sessionId`, just doesn't link anywhere yet
- [ ] Performance
- [ ] Release
