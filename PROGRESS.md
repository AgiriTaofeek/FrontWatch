...
- [~] Performance — **SDK side done**, Go/dashboard side not yet (same split as Network's #15/#16/#17)
  ...
  - [ ] Go ingestion currently **rejects** performance events (`Event.Validate()` only accepts `error`/`network`) — fails gracefully via partial acceptance, not a crash. Next chunk: Go domain/worker/ClickHouse write path + a dashboard surface for performance data
- [ ] Release
