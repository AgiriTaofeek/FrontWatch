...
- [x] CI: `services:` can't override a container's startup command (needed for Redpanda's required flags) — used the official `redpanda-data/github-action` instead of a `services:` entry, same root cause as the local docker-compose bug just found
